...
def main():
    ...
    # 기존의 데이터와 AI HUB에서 받은 외부 데이터를 합쳐서 새로운 데이터셋을 만듭니다.
    datasets1 = load_from_disk(data_args.dataset_name)
    print(f"basic datasets:\n{datasets1}")

    datasets1_train = pd.DataFrame(datasets1['train'])
    datasets1_valid = pd.DataFrame(datasets1['validation'])

    # 이 경로는 각자 AI HUB에서 다운 받으신 'ko_wiki_v1_squad.json'의 경로를 써주셔야 합니다.
    with open("../data/external_dataset/ko_wiki_v1_squad.json") as f:
        datasets2 = json.load(f)
    
    def change_architecture(example):
        output = {'title': example['title']}
        output['context'] = example['paragraphs'][0]['context']
        output['question'] = example['paragraphs'][0]['qas'][0]['question']
        output['id'] = example['paragraphs'][0]['qas'][0]['id']
    ...
    changed_datasets = list(map(change_architecture, datasets2['data']))
    ...
    datasets2 = get_data_frame(changed_datasets)
    ...
    train_dataset = pd.concat([datasets1_train, datasets2_train], ignore_index=True)
    valid_dataset = pd.concat([datasets1_valid, datasets2_valid], ignore_index=True)
...
def run_mrc(
    ...
    # training_args 설정
    # 이 부분은 각자 하이퍼 파라미터 설정을 해주시면 됩니다.
